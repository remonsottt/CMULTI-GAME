// HUD overlay — runs on top of both DASH and RTP zones
export class HUDScene extends Phaser.Scene {
    ...
    create() {
        ...
        // ── LEFT SIDE ──────────────────────────────────────────────────
        this.zoneLbl = this.add.text(12, 10, '', {
            fontSize: '13px', color: '#ffffff',
            backgroundColor: '#00000099', padding: { x: 6, y: 3 }
        }).setDepth(100);

        // Buffer bar (DASH) / Jitter meter (RTP)
        ...
        this.barLabel = this.add.text(12, 62, 'Buffer: 0/7', {
            fontSize: '10px', color: '#cccccc'
        }).setDepth(100);

        // Quality badge
        this.qualityBadge = this.add.text(12, 80, '360p', {
            fontSize: '16px', color: '#E24B4A', fontStyle: 'bold',
            backgroundColor: '#00000099', padding: { x: 6, y: 3 }
        }).setDepth(100);

        // ── RIGHT SIDE ─────────────────────────────────────────────────
        this.abrPanel = this.add.text(W - 12, 10, '', {
            fontSize: '10px', color: '#ffffff',
            backgroundColor: '#00000099', padding: { x: 6, y: 3 },
            align: 'right', wordWrap: { width: 220 }
        }).setOrigin(1, 0).setDepth(100);

        this.lossPanel = this.add.text(W - 12, 100, '', {
            fontSize: '10px', color: '#EF9F27',
            backgroundColor: '#00000099', padding: { x: 6, y: 3 },
            align: 'right'
        }).setOrigin(1, 0).setDepth(100);

        this.igmpStatus = this.add.text(W - 12, 125, 'Channel: — | TTL: 1', {
        ...
        this.igmpLogText = this.add.text(8, H - 37, '', {
            fontSize: '9px', color: '#378ADD', wordWrap: { width: W * 0.6 }
        }).setDepth(100);

        this.controlsText = this.add.text(W - 8, H - 37, this._controlsHint(), {
            fontSize: '9px', color: '#aaaaaa', align: 'right'
        }).setOrigin(1, 0).setDepth(100);
    ...
    _controlsHint() {
        if (this.zone === 'DASH') {
            return '← → Move  |  SPACE Jump  |  F Fast-Fwd';
        }
        return '← → Move  |  SPACE Jump  |  1/2/3 Channel';
    }
    ...
    _onHudUpdate(data) {
        if (data.zone) {
            this.zone = data.zone;
            this.updateZoneLabel();
            this.controlsText.setText(this._controlsHint());
        }

        // Buffer / Jitter bar
        if (data.zone === 'DASH' || (data.buffer !== undefined)) {
            ...
            this.barSegments.forEach((seg, i) => {
                seg.setFillStyle(i < filled ? col : 0x333333);
            });
        ...
        if (data.zone === 'RTP' && data.jitter !== undefined) {
        ...
        // ABR decision panel
        if (data.abrDecision) {
            this.abrPanel.setText(`ABR:\n${data.abrDecision}`);
        }

        // Loss counter
        if (data.loss !== undefined) {
            this.lossPanel.setText(`Loss: ${data.loss}`);
        }

        // IGMP status
        if (data.igmpStatus) {
            this.igmpStatus.setText(data.igmpStatus);
        }
    }

    _onIgmpLog(log) {
        this.igmpLogText.setText(log.slice(-3).join('\n'));
    }
