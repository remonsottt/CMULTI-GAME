...
export class DASHScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.abrText = this.add.text(10, 105, '', {
            fontSize: '10px', color: '#222233', backgroundColor: '#ffffffdd', padding: { x: 5, y: 3 }
        }).setScrollFactor(0).setDepth(88);

        this.cursors = this.input.keyboard.createCursorKeys();
        this.keyF    = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.F);
        this.keyESC  = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.ESC);
        this._showPopup('Collect the MPD Scroll\nto reveal platforms!', 3500);
        this._updateHUD();
    }
    ...
    _buildPlatforms(H) {
        ...
        for (let i = 0; i < TOTAL_SEGS; i++) {
            ...
            this.platforms.push(this._makePlatform(x, py, SEGMENT_W - 12, PLATFORM_H, i));
        }
    }

    _makePlatform(x, y, w, h, segIdx) {
        const gfx = this.add.graphics().setDepth(2).setAlpha(0);
    ...
    _renderPlatformGfx(gfx, x, y, w, h, quality, visible) {
        ...
        gfx.clear();
        gfx.fillStyle(col, 1); gfx.fillRoundedRect(x, y, w, h, 4);
        gfx.fillStyle(0xffffff, 0.18); gfx.fillRoundedRect(x + 2, y + 2, w - 4, 6, 2);
    }
    ...
    _buildMPD() {
        ...
        this.mpdAlive = true; this.mpdX = mx; this.mpdY = my;
        ...
        this.tweens.add({ targets: [this.mpdGfx, this.mpdLbl], y: '-=7', duration: 700, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
    }
    ...
    _buildEndServer(H) {
        ...
        g.fillStyle(0x1D9E75, 1); g.fillRect(ex - 22, this.groundY - 90, 44, 90);
    ...
    _handleInput(dt) {
        ...
        if (Phaser.Input.Keyboard.JustDown(this.keyF) && this.fastFwdCD <= 0) this._doFastForward();
    ...
    _applyPhysics(dt) {
        ...
        for (const plat of this.platforms) {
            const inX = this.playerX + PLAYER_W / 2 > plat.x && this.playerX - PLAYER_W / 2 < plat.x + plat.w;
            ...
            if (inX && wasAbove && nowAt && this.velY >= 0) {
                this.playerY = plat.y; this.velY = 0; this.onGround = true; break;
            }
        }
        ...
        if (this.onGround && Math.abs(this.velX) > 0 && this.hasMPD) {
            ...
            if (this.distTraveled >= SEGMENT_W) { this.distTraveled -= SEGMENT_W; this._drainBuffer(1); }
        }
    }

    _drainBuffer(amount) {
        const mult = this.stormActive ? 2 : 1;
        this.bufferFill = Math.max(0, this.bufferFill - amount * mult);
        if (this.bufferFill <= 0 && !this.frozen) this._triggerFreeze();
    }

    _triggerFreeze() {
        this.frozen = true; this.freezeTimer = 2000; this.bufferEvents++;
        console.log(`[DASH] BUFFER UNDERRUN #${this.bufferEvents}`);
        this._showPopup(`⚠ BUFFERING… (${this.bufferEvents}/3 events)`, 2000);
        if (this.bufferEvents >= 3) this.time.delayedCall(2100, () => this._gameOver());
    }

    _handleFreeze(delta) {
        if (!this.frozen) return;
        this.freezeTimer -= delta;
        if (this.freezeTimer <= 0) { this.frozen = false; this.bufferFill = 1; }
    }

    _checkMPDPickup() {
        if (this.hasMPD || !this.mpdAlive) return;
        if (Math.abs(this.playerX - this.mpdX) < 28 && Math.abs(this.playerY - this.mpdY) < 36) {
            this.hasMPD = true; this.mpdAlive = false;
            this.mpdGfx.setVisible(false); this.mpdLbl.setVisible(false);
            this.bufferFill = 4;
            console.log('[DASH] MPD collected — qualities: 360p / 720p / 1080p');
            this._showPopup('MPD loaded!\nQualities: 360p / 720p / 1080p\nPlatforms revealed!', 3500);
            this.platforms.forEach(p => {
                p.gfx.setAlpha(1); p.lbl.setVisible(true);
    ...
    _checkSegmentEntry() {
        ...
        this.bufferFill = Math.min(MAX_BUFFER, this.bufferFill + stormPenalty);
        const ahead = seg + 3;
        if (ahead < TOTAL_SEGS) this._recolorPlatform(this.platforms[ahead], dlQuality);
        console.log(`[DASH] Seg ${seg} | dl=${dlQuality} | buf=${this.bufferFill.toFixed(1)}/${MAX_BUFFER}`);
        this._runABR();
    }

    _runABR() {
        this.abr.computeNextQuality(this.bufferFill / MAX_BUFFER);
        this.abrText.setText(this.abr.lastDecision);
    }

    _doFastForward() {
        this.playerX += 5 * SEGMENT_W; this.bufferFill = MAX_BUFFER * 0.05;
        this.fastFwdSegs = 3; this.fastFwdCD = 5;
        this.abr.fastForward();
        this._showPopup('⚡ Fast-forward!\nBuffer→5% | Next 3 segs: 360p', 2500);
        this.cameras.main.flash(250, 255, 255, 255);
    }

    _handleStorm(delta) {
        this.stormTimer -= delta;
        if (!this.stormActive && this.stormTimer <= 0) this._startStorm();
        else if (this.stormActive && this.stormTimer <= 0) this._endStorm();
        if (this.stormActive) this._drawStorm();
    }

    _startStorm() {
        this.stormActive = true; this.stormTimer = 8000;
        this.abr.setCongestion(true); this.stormOverlay.setAlpha(0.32);
        for (let i = 0; i < 22; i++) this.stormParticles.push({
            x: Phaser.Math.Between(0, this.scale.width),
            y: Phaser.Math.Between(0, this.scale.height * 0.7),
            r: Phaser.Math.Between(5, 14), spd: Phaser.Math.Between(40, 100),
    ...
    _endStorm() {
        this.stormActive = false; this.stormTimer = 20000; this.stormParticles = [];
        this.abr.setCongestion(false); this.stormOverlay.setAlpha(0); this.stormGfx.clear();
    }

    _drawStorm() {
        const g = this.stormGfx; g.clear();
    ...
    _drawPlayer() {
        ...
        g.fillStyle(this.frozen ? 0x7799ee : 0x2255cc, 1);
        g.fillRoundedRect(-PLAYER_W / 2, -PLAYER_H, PLAYER_W, PLAYER_H, 5);
        g.fillStyle(0xffffff, 0.22); g.fillRoundedRect(-PLAYER_W / 2 + 3, -PLAYER_H + 3, PLAYER_W - 6, 8, 3);
        ...
        g.fillStyle(0xffffff, 0.25); g.fillRoundedRect(-7, -10, 14, 8, 2);
        if (this.frozen) { g.lineStyle(2, 0xaaccff, 0.8); g.strokeRoundedRect(-PLAYER_W / 2 - 2, -PLAYER_H - 2, PLAYER_W + 4, PLAYER_H + 4, 6); }
    }

    _checkEndServer() {
        if (Math.abs(this.playerX - this.endServerX) < 35 && !this.levelComplete) {
            this.levelComplete = true;
            const msg = this.bufferEvents < 3
                ? `🎉 LEVEL COMPLETE!\nBuffer events: ${this.bufferEvents}/3\nYou survived DASH Land!`
                : `Level done — ${this.bufferEvents} buffer events (max 3)`;
            this._showPopup(msg, 99999);
            this.time.delayedCall(4500, () => { this.scene.stop('HUDScene'); this.scene.start('MenuScene'); });
        }
    }
    ...
    _gameOver() {
        if (this.levelComplete) return;
        this.levelComplete = true;
        this._showPopup('💀 GAME OVER\n3 buffer events reached!', 4000);
        this.time.delayedCall(4200, () => { this.scene.stop('HUDScene'); this.scene.start('MenuScene'); });
    }
    ...
    _updateHUD() {
        this.game.events.emit('hudUpdate', {
            zone: 'DASH', buffer: this.bufferFill / MAX_BUFFER,
